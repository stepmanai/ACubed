...
class IngestionService:
    ...
    def sync_songlist(self):
        ...
        exists = self.storage.table_exists(
        ...
        previous = self.storage.read_table(
            self.table_config.songlist
        ).df()
        ...
        changed = changed[
            changed["swf_version_old"]
            != changed["swf_version_new"]
        ]

        new_songs = df[
            ~df["id"].isin(previous["id"])
        ]

        deleted = previous[
            ~previous["id"].isin(df["id"])
        ]
    ...
    def sync_charts(
        ...
        chart_data = (
            self.api_client
            .fetch_charts_parallel(song_ids)
        )
    ...
    def sync_playlist(self):
        playlist = (
            self.api_client.fetch_playlist()
        )
